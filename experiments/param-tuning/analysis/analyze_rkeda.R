...
data.file <- "../results/results_rkeda.csv"

## =========================== ##

data <- read.csv(data.file)

## ony select PFSP instances
data <- data[unlist(lapply(data$instance, (function (x) grepl("tai", x)))),]

## obtain correct trunc size param
data$trunc.size <- data$trunc.size / data$pop.size
...
for (ps in pop.sizes) {
    for (ts in trunc.sizes) {
        ...
        for (inst in instances) {
            values <- data[(data$pop.size == ps & data$trunc.size == ts & data$instance == inst),]$best.fitness
...
bayesianNES_IO<-ggplot(dataBayesian_2, aes(y=Expected, ymin=Lower_bound, ymax=Upper_bound, x=Setting)) + geom_errorbar() + geom_point(col="darkgreen", size=2) +  theme_bw() + coord_flip() + labs(y="Probability of winning")+ggtitle(label = "RK-EDA - PFSP")

ggsave("rk-eda.pdf") #, width=200, height=250, units="mm")
