...
data.file <- "../results/results_pleda.csv"
...
data$trunc.size <- unlist(lapply(data$trunc.size, (function (x) round(x,1))))
unique(data$trunc.size)
...
bayesianNES_IO<-ggplot(dataBayesian_2, aes(y=Expected, ymin=Lower_bound, ymax=Upper_bound, x=Setting)) + geom_errorbar() + geom_point(col="darkgreen", size=2) +  theme_bw() + coord_flip() + labs(y="Probability of winning")+ggtitle(label = "PLEDA - LOP")

ggsave("pleda.pdf") #, width=200, height=250, units="mm")
